...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            assert(self.get_owner() == caller, 'NOT_OWNER');
            let in_stock = self.get_stock(product);

            self.inventory.write(product, in_stock + new_stock)
        }
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            let in_stock = self.inventory.read(product);
            assert(in_stock >= quantity, 'NOT_ENOUGH_FOR_PURCHASE');

            self.inventory.write(product, in_stock - quantity);
        }
